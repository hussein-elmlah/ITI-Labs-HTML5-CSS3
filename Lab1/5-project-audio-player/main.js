...
const speedLevel = document.getElementById('speed');
const durationLevel = document.getElementById('duration');
...
volumeLevel.addEventListener('input', adjustVolume);
speedLevel.addEventListener('input', adjustSpeed);
durationLevel.addEventListener('input', adjustDuration);
...
function adjustSpeed() {
    const speed = speedLevel.value / 50;
    audioPlayer.playbackRate = speed;
}

function adjustDuration() {
    const duration = (durationLevel.value / 100) * audioPlayer.duration;
    audioPlayer.currentTime = duration;
}

function updateDuration() {
    const currentDuration = (audioPlayer.currentTime / audioPlayer.duration) * 100;
    durationLevel.value = currentDuration;
}
